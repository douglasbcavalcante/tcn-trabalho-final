...
DATA_FILES = ["sub_1s.csv","sub_01s.csv","sub_001.csv"];
DF_STEP = [1,0.01,0.001];
//HPA_FILES = ['Teste 1 - 1 ESP32 - pub 1 s - HPA.csv' 'Teste 2 - 1 ESP32 - pub 0.01s - HPA.csv' 'Teste 3 - 1 ESP32 - pub 0.1s - HPA.csv' 'Teste 4 - 2 ESP32 - pub 0.1s - HPA.csv'];
...
for i = 1:size(DATA_FILES)(2)
    ...
    disp(f)
    dados =  read_csv(f);
    send_time = datetime(dados(:,1));
    rec_time = datetime(dados(:,2));
    ...
    if (4 == size(dados)(2))
        ...
        for j = 1:size(ips)(1)
            ...
            //cpu_load = strtod(hpa_data(:,2));
            //pod_number = strtod(hpa_data(:,3));
            //time_mark = datetime(hpa_data(:,6));
            plot_and_save_sendXrec(i+10*j+200, send_time, rec_time, filename + " - sendXrec");
       ...
       // cpu_load = strtod(hpa_data(:,2));
        //pod_number = strtod(hpa_data(:,3));
        //time_mark = datetime(hpa_data(:,6));
        plot_and_save_sendXrec(i+200, send_time, rec_time,DATA_FILES(i) + " - sendXrec");
